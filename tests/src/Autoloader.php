 ...
 * Autoloader file
 * Imported from
 * https://github.com/alleyinteractive/wordpress-autoloader/blob/main/src/class-autoloader.php
...
class Autoloader {
	 ...
	 * @var string
	 */
	protected string $namespace_str;

	/**
	 * Root path of the namespace_str to load from.
	 *
	 * @var string
	 */
	 ...
	 */
	protected array $missing_classes = array();
	 ...
	 *
	 * @param string $namespace_str Namespace to autoload.
	 * @param string $root_path Path in which to look for files.
	 * @return static Function for spl_autoload_register().
	 */
	public static function generate( string $namespace_str, string $root_path ): callable {
		return new static( $namespace_str, $root_path );
	}

	/**
	 * Constructor.
	 *
	 * @param string $namespace_str Namespace to register.
	 * @param string $root_path Root path of the namespace_str.
	 */
	public function __construct( string $namespace_str, string $root_path ) {
		$this->namespace_str = $namespace_str;
	...
	public function __invoke( string $classname ) {
		// Ignore if the base namespace_str doesn't match.
		if ( 0 !== \strpos( $classname, $this->namespace_str ) ) {
			return;
		}
	...
	protected function find_file( string $classname ): ?string {
		...
		// Build the base path relative to the sub-namespace_str.
		$sub_namespace = \substr( \implode( DIRECTORY_SEPARATOR, $parts ), \strlen( $this->namespace_str ) );
